...
## 16. 服务器目录流水线与 crontab

服务器模式采用三个独立阶段，JSON 不和 optimizer 同步产生：
...
/data/01-pdf-input
        ...
        ▼
/data/02-lexoid-tex
        │  tabularx/SyncTeX optimizer cron worker
        ▼
/data/03-review-pending
        │  人工核对并把新 TeX 放入 reviewed 目录
        ▼
/data/04-reviewed-tex
        │  JSON cron worker（唯一 JSON 输入）
        ▼
/data/05-json-output
```

JSON worker 绝不监听 `02-lexoid-tex` 或 `03-review-pending`。只有人工放入
`04-reviewed-tex` 的 TeX 才会生成 JSON。
默认 `ENABLE_JSON_STAGE=false`，因此即使 reviewed 目录已有文件也不会运行
JSON。人工复核流程就绪后，显式改为 `true` 才会启用。
人工核对后请保留 `03-review-pending` 中的 TeX 文件名，再将修订版放入
`04-reviewed-tex`；这样 JSON worker 可以自动找回同名 `.fields.json` 字段注册表，
继续保留 `VALUE_ID` 与语义别名的关联。

所有 worker 都递归扫描并保留输入根目录下的相对路径。例如：

```text
PDF_INPUT_HOST_DIR/客户A/2026/form.pdf
  -> 02-lexoid-tex/客户A/2026/form.tex
  -> 03-review-pending/客户A/2026/form.optimized.tex
  -> 04-reviewed-tex/客户A/2026/form.optimized.tex
  -> 05-json-output/客户A/2026/form.optimized.json
```

`PDF_INPUT_HOST_DIR` 可以是只读的现有 PDF 目录树，`PIPELINE_HOST_ROOT`
...
```bash
cp .env.example .env
mkdir -p data/{01-pdf-input,02-lexoid-tex,03-review-pending,04-reviewed-tex,05-json-output,state,logs}
docker compose build pipeline-cron
```

`.env` 可以配置全部输入、输出、状态和日志目录。`LEXOID_IMAGE`
指定已有的 Lexoid 镜像，构建时会把 Lexoid 和 optimizer 合并到同一个
cron worker 镜像。`LEXOID_COMMAND` 是无 shell argv 模板，支持 `{input}`、
`{output}`、`{model}`、`{start_page}` 占位符。

Optimizer 通过 `OPTIMIZER_EXTRA_ARGS=--llm-model <model> --name-cache
/data/state/texopt-names.json` 启用 LLM 语义命名。命名缓存持久化在 `/data/state`，
同一表格重试时会复用稳定结果。LLM 只生成 `semantic_alias`，不改动
Lexoid `VALUE_ID` 或字段值。

输入 LaTeX 的结构诊断只记录为 `SYNTAX_INPUT` 和 `SYNTAX_INPUT_DEFERRED`，
不作为进入 optimizer 的前置拦截条件。最终输出仍执行严格语法检查，
无效的优化结果不会被标记为完成。

`--llm-syntax-repair` 会在表格探针前按 `LEXOID_PAGE_COMPLETED` 物理页分块，
默认将连续 5 页合并为一个整体上下文（可用
`--syntax-repair-batch-pages` 调整），
模型只返回带批内行号的最小修改补丁；未提及的源码由程序逐字节保留，
避免长文复述造成字段值、页标记或未修改页面丢失。
合法的模型补丁全部接受；内容保护检查仅作为 `audit_flags` 写入日志，
不再触发 rejected。语法修复稿默认只在内存中交给后续阶段，最终只交付
`.optimized.tex`；需要检查模型修改时，可显式传入
`--syntax-repair-output` 另存中间快照。
让 LLM 修复未平衡括号、环境、表格 `&`/列数、行结束和中间
`\end{document}`。接受修复前必须通过不可变校验：`VALUE_ID`、字段值、
分页标记和中文可见文本不得改变。已接受的页缓存在
`/data/state/texopt-syntax-repairs.json`，重试不重复调用。

### 16.2 安装 crontab

将 [crontab.example](crontab.example) 中的项目目录改成服务器实际路径，然后：
...
```cron
* * * * * cd /opt/lexoid-pipeline && /usr/bin/docker compose run --rm pipeline-cron >> /opt/lexoid-pipeline/data/logs/cron.log 2>&1
```

每次容器只执行 `texopt-pipeline --once`，扫描一轮后退出。内部 `flock` 会跳过
重叠的 cron 实例。

### 16.3 状态、去重与中断恢复

- SQLite 状态文件默认是 `/data/state/pipeline.sqlite3`，使用 WAL 和同步提交；
- 去重键为 `stage + 绝对路径 + SHA-256`；同一个文件版本完成后不会重复运行；
- 同名文件内容改变会产生新指纹，作为新版本重新处理；
- 容器中断时的 `running` 项，下次 cron 自动恢复为 `retry`；
- 失败按指数退避重试，`MAX_ATTEMPTS` 默认为 `3`；`0` 可显式启用无限重试，仅建议用于非确定性外部故障；
- Lexoid 的 `.partial.tex` 若含 `LEXOID_PAGE_COMPLETED: n/total`，下次对该
  partial 文件原位续写并传入 `--start-page n+1`；
- 每个完成产物旁生成 `.pipeline.done` 标记，同时数据库记录完成状态。

查看历史状态：

```bash
docker compose run --rm pipeline-cron texopt-pipeline --status
```

### 16.4 日志

- `/data/logs/pipeline.jsonl`：跨 cron 追加的全局结构化事件；
- `/data/logs/lexoid/<relative-path>.pdf.log`：Lexoid 原始 stdout/stderr；
- `/data/logs/optimizer/<relative-path>.tex.texopt.log`：optimizer 详细事件；
- `/data/logs/optimizer-process/<relative-path>.tex.log`：optimizer 进程输出；
- `data/logs/cron.log`：crontab/Docker 启动日志。

实时观察：

```bash
tail -F data/logs/pipeline.jsonl data/logs/cron.log
```
